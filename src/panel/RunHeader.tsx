...
export function RunHeader({ run }: { run: RunModel }) {
  ...
  return (
    <div className="agent-frame min-w-0">
      <header className="agent-frame-inner p-3 sm:p-4 md:p-5">
        <div className="min-w-0 space-y-3">
          <div className="-mx-1 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-2 px-1">
            <LiveDot live={live} />
            <span
              className={`shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider ring-1 ${badge[run.status]}`}
            >
              {label[run.status]}
            </span>
            {run.runId ? (
              <span className="max-w-[min(100%,14rem)] truncate rounded-md border border-zinc-200 bg-zinc-50 px-2 py-0.5 font-mono text-[11px] text-violet-700 dark:border-zinc-700 dark:bg-zinc-950 dark:text-violet-300 sm:max-w-none">
                {run.runId}
              </span>
            ) : null}
            <span className="shrink-0 rounded-md border border-zinc-200 bg-white px-2 py-0.5 font-mono text-[11px] tabular-nums text-zinc-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1.5 break-words text-base font-semibold leading-snug tracking-tight text-zinc-900 dark:text-zinc-50 sm:text-lg">
              {run.query}
            </p>
          </div>
        </div>
        {run.status === 'failed' && run.errorMessage ? (
          <div className="mt-4 rounded-xl border border-rose-200/90 bg-rose-50/90 px-3 py-2.5 dark:border-rose-900/50 dark:bg-rose-950/50">
            ...
            <p className="mt-1 break-words font-mono text-xs leading-relaxed text-rose-900 dark:text-rose-100">
              {run.errorMessage}
            </p>
          </div>
        ) : null}
